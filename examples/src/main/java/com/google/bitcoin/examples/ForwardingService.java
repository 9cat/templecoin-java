...
public class ForwardingService {
	...
	public static void main(String[] args) throws Exception {
		...
		// Figure out which network we should connect to. Each one gets its own
		// set of files.
		...
		forwardingAddress = new Address(params,
				"TkqKdEnMNqTtYeFG5wuJJJJQ1XejpVsLeK");

		// Start up a basic app using a class that automates some boilerplate.
		kit = new WalletAppKit(params, new File("."), filePrefix);

		// if (params == RegTestParams.get()) {
		// Regression test mode is designed for testing and development only, so
		// there's no public network for it.
		// If you pick this mode, you're expected to be running a local
		// "bitcoind -regtest" instance.
		// kit.connectToLocalHost();
		// }

		kit.setPeerNodes(new PeerAddress(InetAddress.getByName("mc.9cat.net"),
				params.getPort()));
		...
		kit.wallet().addEventListener(new AbstractWalletEventListener() {
			...
			public void onCoinsReceived(Wallet w, Transaction tx,
				...
				// Runs in the dedicated "user thread" (see bitcoinj docs for
				// more info on this).
				//
				// The transaction "tx" can either be pending, or included into
				// a block (we didn't see the broadcast).
				BigInteger value = tx.getValueSentToMe(w);
				System.out.println("Received tx for "
						+ Utils.bitcoinValueToFriendlyString(value) + ": " + tx);
				System.out
						.println("Transaction will be forwarded after it confirms.");
				// Wait until it's made it into the block chain (may run
				...
				// be called in onSetupCompleted() above. But we don't do that
				// here to demonstrate the more common
				// case of waiting for a block.
				Futures.addCallback(tx.getConfidence().getDepthFuture(1),
						new FutureCallback<Transaction>() {
							@Override
							public void onSuccess(Transaction result) {
								// "result" here is the same as "tx" above, but
								// we use it anyway for clarity.
								forwardCoins(result);
							}
	...
	private static void forwardCoins(Transaction tx) {
		try {
			...
			final Wallet.SendResult sendResult = kit.wallet().sendCoins(
					kit.peerGroup(), forwardingAddress, amountToSend);
			...
			// We don't use encrypted wallets in this example - can never
			// happen.
			throw new RuntimeException(e);
		} catch (InsufficientMoneyException e) {
			// This should never happen - we're only trying to forward what we
			// received!
			throw new RuntimeException(e);
		}
	}
