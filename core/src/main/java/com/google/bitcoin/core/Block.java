 ...
 * and proves that a difficult calculation was done over its contents. See <a
 * href="http://www.bitcoin.org/bitcoin.pdf">the Bitcoin technical paper</a> for
...
public class Block extends Message {
	 ...
	 */
	public Block(NetworkParameters params, long version,
			Sha256Hash prevBlockHash, Sha256Hash merkleRoot, long time,
	 ...
	 * {@link com.google.bitcoin.core.NetworkParameters#getSubsidyDecreaseBlockCount()}
	 * .
	 * </p>
	 */
	public BigInteger getBlockInflation(long diffTime, long nBits, int nHeight,
			double nFees) {
		// return Utils.toNanoCoins(50, 0).shiftRight(height /
		// params.getSubsidyDecreaseBlockCount());
		return /* Utils.toNanoCoins( */CoinDefinition.GetBlockReward(diffTime,
				nBits, nHeight, nFees)/* , 0) */;
	}
	...
	private void parseHeader() throws ProtocolException {
		...
		hash = new Sha256Hash(Utils.reverseBytes(Utils.doubleDigest(bytes,
				offset, cursor)));
	...
	private void parseTransactions() throws ProtocolException {
		...
		for (int i = 0; i < numTransactions; i++) {
			Transaction tx = new Transaction(params, bytes, cursor, this,
					parseLazy, parseRetain, UNKNOWN_LENGTH);
		...
		// be set at the end of parseLight().
		// If this is a genuine lazy parse then length must have been provided
		// to the constructor.
		transactionsParsed = true;
	...
	protected void parseLite() throws ProtocolException {
		...
		if (length == UNKNOWN_LENGTH) {
			...
			transactionBytesValid = !transactionsParsed || parseRetain
					&& length > HEADER_SIZE;
		}
		headerBytesValid = !headerParsed || parseRetain
				&& length >= HEADER_SIZE;
	}
	 ...
	 * separating them. There are many cases where a user may need to access or
	 * change one or the other but not both.
	 * 
	 * With this in mind we ignore the inherited checkParse() and unCache()
	 * methods and implement a separate version of them for both header and
	 * transactions.
	 * 
	...
	/**
	 * In lazy parsing mode access to getters and setters may throw an unchecked
	 * LazyParseException. If guaranteed safe access is required this method
	 * will force parsing to occur immediately thus ensuring LazyParseExeption
	 * will never be thrown from this Message. If the Message contains child
	 * messages (e.g. a Block containing Transaction messages) this will not
	 * force child messages to parse.
	 * 
	...
	void writeHeader(OutputStream stream) throws IOException {
		...
		if (headerBytesValid && bytes != null
				&& bytes.length >= offset + HEADER_SIZE) {
	...
	public byte[] bitcoinSerialize() {
		...
		ByteArrayOutputStream stream = new UnsafeByteArrayOutputStream(
				length == UNKNOWN_LENGTH ? HEADER_SIZE
						+ guessTransactionsLength() : length);
		try {
			writeHeader(stream);
	...
	private Sha256Hash calculateHash() {
		try {
			ByteArrayOutputStream bos = new UnsafeByteArrayOutputStream(
					HEADER_SIZE);
			writeHeader(bos);
			return new Sha256Hash(Utils.reverseBytes(doubleDigest(bos
					.toByteArray())));
		} catch (IOException e) {
	...
	public String toString() {
		...
		if (transactions != null && transactions.size() > 0) {
			s.append("   with ").append(transactions.size())
	...
	private boolean checkProofOfWork(boolean throwException)
		...
		if (h.compareTo(target) > 0) {
			...
			if (throwException)
				throw new VerificationException("Hash is higher than target: "
						+ getScryptHashAsString() + " vs "
						+ target.toString(16));
			else
				return false;
		}
	...
	private void checkSigOps() throws VerificationException {
		...
		if (sigOps > MAX_BLOCK_SIGOPS)
			throw new VerificationException(
					"Block had too many Signature Operations");
	}
	...
	private List<byte[]> buildMerkleTree() {
		...
		for (int levelSize = transactions.size(); levelSize > 1; levelSize = (levelSize + 1) / 2) {
			...
			for (int left = 0; left < levelSize; left += 2) {
				...
				byte[] leftBytes = Utils.reverseBytes(tree.get(levelOffset
						+ left));
				byte[] rightBytes = Utils.reverseBytes(tree.get(levelOffset
						+ right));
				tree.add(Utils.reverseBytes(doubleDigestTwoBuffers(leftBytes,
	...
	private void checkTransactions() throws VerificationException {
		...
		for (int i = 1; i < transactions.size(); i++) {
			if (transactions.get(i).isCoinBase())
				throw new VerificationException("TX " + i
	 ...
	 * encoded <b>in compact form</b>. The {@link BlockChain} verifies that this
	 ...
	 * {@link com.google.bitcoin.core.Block#getDifficultyTargetAsInteger()}.
	 * Note that this is <b>not</b> the same as the difficulty value reported by
	 * the Bitcoin "getdifficulty" RPC that you may see on various block
	 * explorers. That number is the result of applying a formula to the
	 * underlying difficulty to normalize the minimum to 1. Calculating the
	...
	void addCoinbaseTransaction(byte[] pubKeyTo, BigInteger value) {
		...
		coinbase.addInput(new TransactionInput(params, coinbase, new byte[] {
				(byte) txCounter, (byte) (txCounter++ >> 8) }));
		coinbase.addOutput(new TransactionOutput(params, coinbase, value,
				ScriptBuilder.createOutputScript(new ECKey(null, pubKeyTo))
						.getProgram()));
	 ...
	 */
	Block createNextBlock(@Nullable Address to,
			@Nullable TransactionOutPoint prevOut, long time, byte[] pubKey,
		...
		if (to != null) {
			...
			if (prevOut == null) {
				input = new TransactionInput(params, t,
	...
	public Block createNextBlock(@Nullable Address to,
		...
		return createNextBlock(to, prevOut, Utils.now().getTime() / 1000,
				EMPTY_BYTES, Utils.toNanoCoins(50, 0));
	}

	@VisibleForTesting
	public Block createNextBlock(@Nullable Address to, BigInteger value) {
		return createNextBlock(to, null, Utils.now().getTime() / 1000,
				EMPTY_BYTES, value);
	}
	...
	public Block createNextBlockWithCoinbase(byte[] pubKey,
			BigInteger coinbaseValue) {
		return createNextBlock(null, null, Utils.now().getTime() / 1000,
				pubKey, coinbaseValue);
	}
