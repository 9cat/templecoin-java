 ...
 * with the downside being a larg cost in system resources. Fully verifying
 ...
 * nevertheless provides the same security guarantees as a regular Satoshi
 * client does.
 * </p>
 */
public class FullPrunedBlockChain extends AbstractBlockChain {
	private static final Logger log = LoggerFactory
			.getLogger(FullPrunedBlockChain.class);
	...
	/**
	 * Constructs a BlockChain connected to the given list of wallets and a
	 * store.
	 */
	public FullPrunedBlockChain(NetworkParameters params,
			List<BlockChainListener> listeners, FullPrunedBlockStore blockStore)
	...
	@Override
	protected StoredBlock addToBlockStore(StoredBlock storedPrev, Block header,
			TransactionOutputChanges txOutChanges) throws BlockStoreException,
			VerificationException {
		StoredBlock newBlock = storedPrev.build(header);
		blockStore.put(newBlock, new StoredUndoableBlock(newBlock.getHeader()
				.getHash(), txOutChanges));
		return newBlock;
	}
	...
	ExecutorService scriptVerificationExecutor = Executors
	...
	private static class Verifier implements Callable<VerificationException> {
		...
		public Verifier(final Transaction tx,
				final List<Script> prevOutScripts,
				final boolean enforcePayToScriptHash) {
			this.tx = tx;
			...
			try {
				...
				for (int index = 0; index < tx.getInputs().size(); index++) {
					tx.getInputs()
							.get(index)
							.getScriptSig()
							.correctlySpends(tx, index, prevOutIt.next(),
									enforcePayToScriptHash);
				}
	...
	protected TransactionOutputChanges connectTransactions(int height,
			Block block, StoredBlock storedBlock) throws VerificationException,
			BlockStoreException {
		...
		if (!params.passesCheckpoint(height, block.getHash()))
			throw new VerificationException(
					"Block failed checkpoint lockin at " + height);
		...
		if (scriptVerificationExecutor.isShutdown())
			scriptVerificationExecutor = Executors.newFixedThreadPool(Runtime
		...
		try {
			if (!params.isCheckpoint(height)) {
				...
				// transaction. They are all in the
				// checkpoints list and we therefore only check non-checkpoints
				// for duplicated transactions here. See the
				...
				for (Transaction tx : block.transactions) {
					...
					// the tx already. Either the block is
					// being added twice (bug) or the block is a BIP30 violator.
					if (blockStore.hasUnspentOutputs(hash, tx.getOutputs()
							.size()))
						throw new VerificationException(
								"Block failed BIP30 test!");
					if (enforcePayToScriptHash) // We already check non-BIP16
												// sigops in
			...
			for (final Transaction tx : block.transactions) {
				...
				if (!isCoinBase) {
					// For each input of the transaction remove the
					// corresponding output from the set of unspent
					// outputs.
					for (int index = 0; index < tx.getInputs().size(); index++) {
						TransactionInput in = tx.getInputs().get(index);
						StoredTransactionOutput prevOut = blockStore
								.getTransactionOutput(in.getOutpoint()
										.getHash(), in.getOutpoint().getIndex());
						if (prevOut == null)
							throw new VerificationException(
									"Attempted to spend a non-existent or already spent output!");
						// Coinbases can't be spent until they mature, to avoid
						// re-orgs destroying entire transaction
						// chains. The assumption is there will ~never be
						// re-orgs deeper than the spendable coinbase
						// chain depth.
						if (height - prevOut.getHeight() < params
								.getSpendableCoinbaseDepth())
							throw new VerificationException(
									"Tried to spend coinbase at depth "
											+ (height - prevOut.getHeight()));
						// TODO: Check we're not spending the genesis
						// transaction here. Satoshis code won't allow it.
						valueIn = valueIn.add(prevOut.getValue());
						if (enforcePayToScriptHash) {
							if (new Script(prevOut.getScriptBytes())
									.isPayToScriptHash())
								sigOps += Script.getP2SHSigOpCount(in
										.getScriptBytes());
							if (sigOps > Block.MAX_BLOCK_SIGOPS)
								throw new VerificationException(
										"Too many P2SH SigOps in block");
						}

						prevOutScripts
								.add(new Script(prevOut.getScriptBytes()));

						// in.getScriptSig().correctlySpends(tx, index, new
						// Script(params, prevOut.getScriptBytes(), 0,
						// prevOut.getScriptBytes().length));
				...
				for (TransactionOutput out : tx.getOutputs()) {
					...
					// it can be consumed in future.
					StoredTransactionOutput newOut = new StoredTransactionOutput(
							hash, out.getIndex(), out.getValue(), height,
							isCoinBase, out.getScriptBytes());
					blockStore.addUnspentTransactionOutput(newOut);
					txOutsCreated.add(newOut);
				}
				// All values were already checked for being non-negative (as it
				// is verified in Transaction.verify())
				// but we check again here just for defence in depth.
				// Transactions with zero output value are OK.
				if (valueOut.compareTo(BigInteger.ZERO) < 0
						|| valueOut.compareTo(params.MAX_MONEY) > 0)
					throw new VerificationException(
							"Transaction output value out of rage");
				if (isCoinBase) {
					coinbaseValue = valueOut;
				} else {
					if (valueIn.compareTo(valueOut) < 0
							|| valueIn.compareTo(params.MAX_MONEY) > 0)
						throw new VerificationException(
				...
				if (!isCoinBase && runScripts) {
					...
					FutureTask<VerificationException> future = new FutureTask<VerificationException>(
							new Verifier(tx, prevOutScripts,
									enforcePayToScriptHash));
			...
			long diffTime = block.getTimeSeconds()
					- storedBlock.getHeader().getTimeSeconds();
			long nBit = block.getDifficultyTarget();

			if (totalFees.compareTo(params.MAX_MONEY) > 0
					|| block.getBlockInflation(diffTime, nBit, height,
							totalFees.doubleValue()).add(totalFees)
	 ...
	 */
	protected synchronized TransactionOutputChanges connectTransactions(
			StoredBlock newBlock) throws VerificationException,
			BlockStoreException, PrunedException {
		checkState(lock.isHeldByCurrentThread());
		if (!params.passesCheckpoint(newBlock.getHeight(), newBlock.getHeader()
				.getHash()))
			throw new VerificationException(
					"Block failed checkpoint lockin at " + newBlock.getHeight());

		blockStore.beginDatabaseBatchWrite();
		StoredUndoableBlock block = blockStore.getUndoBlock(newBlock
				.getHeader().getHash());
		if (block == null) {
			// We're trying to re-org too deep and the data needed has been
			// deleted.
			blockStore.abortDatabaseBatchWrite();
		...
		try {
			...
			if (transactions != null) {
				...
				LinkedList<StoredTransactionOutput> txOutsCreated = new LinkedList<StoredTransactionOutput>();
				long sigOps = 0;
				final boolean enforcePayToScriptHash = newBlock.getHeader()
						.getTimeSeconds() >= NetworkParameters.BIP16_ENFORCE_TIME;
				...
				for (final Transaction tx : transactions) {
					...
					if (!isCoinBase) {
						for (int index = 0; index < tx.getInputs().size(); index++) {
							final TransactionInput in = tx.getInputs().get(
									index);
							final StoredTransactionOutput prevOut = blockStore
									.getTransactionOutput(in.getOutpoint()
											.getHash(), in.getOutpoint()
											.getIndex());
							if (prevOut == null)
								throw new VerificationException(
										"Attempted spend of a non-existent or already spent output!");
							if (newBlock.getHeight() - prevOut.getHeight() < params
									.getSpendableCoinbaseDepth())
								throw new VerificationException(
										"Tried to spend coinbase at depth "
												+ (newBlock.getHeight() - prevOut
														.getHeight()));
							valueIn = valueIn.add(prevOut.getValue());
							if (enforcePayToScriptHash) {
								Script script = new Script(
										prevOut.getScriptBytes());
							...
							prevOutScripts.add(new Script(prevOut
									.getScriptBytes()));
					...
					for (TransactionOutput out : tx.getOutputs()) {
						valueOut = valueOut.add(out.getValue());
						StoredTransactionOutput newOut = new StoredTransactionOutput(
								hash, out.getIndex(), out.getValue(),
								newBlock.getHeight(), isCoinBase,
								out.getScriptBytes());
						blockStore.addUnspentTransactionOutput(newOut);
						txOutsCreated.add(newOut);
					}
					// All values were already checked for being non-negative
					// (as it is verified in Transaction.verify())
					// but we check again here just for defence in depth.
					// Transactions with zero output value are OK.
					if (valueOut.compareTo(BigInteger.ZERO) < 0
							|| valueOut.compareTo(params.MAX_MONEY) > 0)
					...
					if (!isCoinBase) {
						// Because correctlySpends modifies transactions, this
						// must come after we are done with tx
						FutureTask<VerificationException> future = new FutureTask<VerificationException>(
								new Verifier(tx, prevOutScripts,
										enforcePayToScriptHash));
						scriptVerificationExecutor.execute(future);
				...
				// TODO confusing implmentation

				if (totalFees.compareTo(params.MAX_MONEY) > 0
						|| newBlock
								.getHeader()
								.getBlockInflation(0, 0, newBlock.getHeight(),
										0.001).add(totalFees)
								.compareTo(coinbaseValue) < 0)
					throw new VerificationException(
							"Transaction fees out of range");
				txOutChanges = new TransactionOutputChanges(txOutsCreated,
						txOutsSpent);
				for (Future<VerificationException> future : listScriptVerificationResults) {
					VerificationException e;
					try {
				...
				if (!params.isCheckpoint(newBlock.getHeight()))
					for (StoredTransactionOutput out : txOutChanges.txOutsCreated) {
						...
						if (blockStore.getTransactionOutput(hash,
								out.getIndex()) != null)
							throw new VerificationException(
									"Block failed BIP30 test!");
					}
	 ...
	 * blocks which are allowed to fail BIP30 must be checkpointed.
	 */
	@Override
	protected void disconnectTransactions(StoredBlock oldBlock)
		...
		blockStore.beginDatabaseBatchWrite();
		try {
			StoredUndoableBlock undoBlock = blockStore.getUndoBlock(oldBlock
					.getHeader().getHash());
			if (undoBlock == null)
